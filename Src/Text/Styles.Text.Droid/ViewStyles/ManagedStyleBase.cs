...
namespace Styles.Text
{
    public abstract class ManagedStyleBase : IManagedStyle
    {
        ...
        ISpanned _attributedValue;
        public ISpanned AttributedValue
        {
            get
            {
                if (_attributedValue == null)
                    _attributedValue = ContainsHtml ? styleInstance.CreateHtmlString(_text, _styleID, CustomTags) : styleInstance.CreateStyledString(Style, _text); ;
